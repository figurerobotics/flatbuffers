...
class PythonGenerator : public BaseGenerator {
 public:
  ...
  void GetVectorLen(const StructDef &struct_def, const FieldDef &field,
    ...
    GenReceiver(struct_def, code_ptr);
    code += namer_.Method(field) + "_length(self)";
  ...
  void GetVectorIsNone(const StructDef &struct_def, const FieldDef &field,
    ...
    GenReceiver(struct_def, code_ptr);
    code += namer_.Method(field) + "_is_none(self)";
  ...
  std::string GenUnionReturnTyping(const std::vector<EnumVal *> &type_vals,
                                   bool add_none = true) const {
  ...
  void GetMemberOfVectorOfUnion(const StructDef &struct_def,
    ...
    code +=
        GenIndents(3) + "type = self." + namer_.Method(field) + "_type_get(j)";
    code += GenIndents(3) + "obj = flatbuffers.table.Table(self._tab.Bytes, x)";
    code += GenIndents(3) + "return " + vectortype.enum_def->name +
            "Creator(type, obj)";
    code += GenIndents(2) + "return None\n\n";

    code += GenIndents(1) + "def " + namer_.Method(field) + "_table";
    if (is_native_table) {
      import_entry = ImportMapEntry{ "flatbuffers.table", "Table" };
    } else {
      return_type = TypeName(field);
  ...
  void GetUnionField(const StructDef &struct_def, const FieldDef &field,
    ...
    code += GenIndents(3) + "return " + field.value.type.enum_def->name +
            "Creator(type, obj)";

    code += GenIndents(2) + "return None\n\n";

    code += GenIndents(1) + "def " + namer_.Method(field) + "_table(self)";
    code += ":";
    code += OffsetPrefix(field, false);
  ...
  void GetVectorOfNonStructAsNumpy(const StructDef &struct_def,
    ...
    GenReceiver(struct_def, code_ptr);
    code += namer_.Method(field) + "_as_numpy(self):";
    if (!IsArray(field.value.type)) {
      code += OffsetPrefix(field, false);
  ...
  void GenVectorInit(const FieldDef &field, std::string *field_type_ptr,
    ...
    if (base_type == BASE_TYPE_STRUCT) {
      ...
      for (auto it = enum_def->Vals().begin(); it != enum_def->Vals().end();
        ...
        switch (ev.union_type.base_type) {
          case BASE_TYPE_STRUCT:
            union_field_type = namer_.ObjectType(*ev.union_type.struct_def);
            if (parser_.opts.include_dependence_headers) {
              auto package_reference = GenPackageReference(ev.union_type);
              union_field_type = package_reference + "." + union_field_type;
              import_list->insert("import " + package_reference);
            }
            break;
          case BASE_TYPE_STRING: union_field_type += "str"; break;
          case BASE_TYPE_NONE: union_field_type += "None"; break;
          default: break;
        }
        field_type += union_field_type + separator_string;
      }

      // Removes the last separator_string.
      field_type.erase(field_type.length() - separator_string.size());
      field_type += "]]";
  ...
  void GenInitialize(const StructDef &struct_def, std::string *code_ptr,
    ...
    code_base +=
        GenIndents(2) + "super.__setattr__(self, '_fb_initialized', False)";
    if (code.empty()) {
    ...
    code_base +=
        GenIndents(4) +
        "raise AttributeError(f'{k} is not a attribute of {type(self)}')";
  ...
  void GenCompareOperator(const StructDef &struct_def,
    ...
    code +=
        GenIndents(3) + "and hasattr(self, '_tab') and hasattr(other, '_tab')";
    for (auto it = struct_def.fields.vec.begin();
         it != struct_def.fields.vec.end(); ++it) {
      auto &field = **it;
      if (field.deprecated) continue;
  ...
  void GenReprObjectAPI(const StructDef &struct_def,
                        std::string *code_ptr) const {
    auto &code = *code_ptr;
    const auto struct_object = namer_.ObjectType(struct_def);
  ...
  void GenSetAttr(std::string *code_ptr) const {
    auto &code = *code_ptr;
    code +=
        GenIndents(1) + "def __setattr__(self, name:str, value:any) -> any:";
    code += GenIndents(2) + "if name != '_tab':";
    code += GenIndents(3) +
            "raise AttributeError(\"The primary flatbuffers API doesn't allow "
            "assignment," +
  ...
  void GenSetAttrObjectAPI(std::string *code_ptr) const {
    auto &code = *code_ptr;
    code +=
        GenIndents(1) + "def __setattr__(self, name:str, value:any) -> any:";
    code += GenIndents(2) +
            "if self._fb_initialized and name not in self.__dict__:";
    code += GenIndents(3) +
            "raise AttributeError(f\"Field '{name}' does not exist on "
            "'{type(self)}'\")";
    code += GenIndents(2) + "super.__setattr__(self, name, value)";
    code += "\n";
  }
  ...
  void GenUnPackForUnionVector(const StructDef &struct_def,
    ...
    code += GenIndents(5) + "self." + field_field + ".append(" + union_type +
            "TCreator(" + "self." + field_field + "_type[i], " + struct_var +
            "." + field_method + "_table_get(i)))";
  }
      ...
      switch (field.value.type.base_type) {
        ...
        case BASE_TYPE_VECTOR: {
          ...
          if (vectortype.base_type == BASE_TYPE_STRUCT) {
            ...
            GenUnPackForUnionVector(struct_def, field, &code);
          } else {
      ...
      switch (field.value.type.base_type) {
        ...
        case BASE_TYPE_VECTOR: {
          ...
          if (vectortype.base_type == BASE_TYPE_STRUCT) {
            ...
            GenPackForUnionVectorField(struct_def, field, &code_prefix, &code);
          } else {
  ...
  void GenStructForObjectAPI(const StructDef &struct_def,
    ...
    GenSetAttrObjectAPI(&code);

    if (parser_.opts.gen_compare) {
      GenCompareOperatorForObjectAPI(struct_def, &code);
    }
  ...
  std::string GenUnionType(const Type &type, uint8_t indents,
  ...
  void GenUnionCreatorForString(const EnumDef &enum_def, const EnumVal &ev,
    ...
    code +=
        GenIndents(1) + "if unionType == " + union_type + "." + variant + ":";
    code +=
        GenIndents(2) + "tab = flatbuffers.table.Table(table.Bytes, table.Pos)";
